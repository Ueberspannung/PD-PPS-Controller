 ...
 * 
 * Modifications KRS:
 * - added defines for SERIAL_TX_BUFFER_SIZE to PD_UFP_Log.cpp, which is 
 * 	 only defined for AVR devices
 * - modified timer function in PD_UFP.cpp due to differen register sizes 
 * 	 in AVR and ARM devices the SW stops executing the FUSB_302_alert 
 * 	 function after 65536 ms, needed to cast expressions in if's to uint16_t
 */
...
class PD_UFP_c
{
    public:
		...
		// check if immedeate service is needed
		bool is_busy(void);
