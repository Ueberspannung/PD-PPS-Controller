...
const uint8_t menu_lcd_c::icon_tab[][ICON_LENGTH] = {   //
...
const char menu_lcd_c::pgmTextFile[] =					"(\x02)=      \x05         "
 ...
 const menu_lcd_c::menu_icon_et menu_lcd_c::pgmIconsSettings[]  =
														{ 	icon_settings,
															icon_ramp,
															icon_switch,
															icon_regulator,
...
const menu_lcd_c::menu_pos_st menu_lcd_c::pgmMenuDelete[]  =
	{	{	menu_item_ok,			menu_item_end, 		menu_mode_select_edit,	   7, 3,     0},
		{	menu_item_cancel,		menu_item_end,		menu_mode_select_edit,	  10, 3,     0},
		{	menu_item_end,  		menu_item_end,		menu_mode_off,	   	   0, 0,     0}};
...
void menu_lcd_c::forceUpdate(void)
{   // forceUpdate
    // set update Flag
    force_update=true;
...
void menu_lcd_c::setRemote(bool bRemote)
{	// setRemote
	if (bRemote)
...
void menu_lcd_c::draw_menu(const char * menuText, const menu_icon_et * iconList)
{	// draw menu
	uint8_t n;
	menu_icon_et icon;
...
// process MenuTable,
void menu_lcd_c::process_button(const menu_pos_st * menuPos)
{
	buttonState.buttonDown=button.down();
	buttonState.buttonHeld=button.long_press();
	buttonState.buttonPressed=button.pressed();
	buttonState.buttonTurns=button.turns();
...
void menu_lcd_c::next_menu_item(const menu_pos_st * menuPos)
...
void menu_lcd_c::set_cursor(uint8_t x, uint8_t y)
{	// set_curosr
...
uint8_t menu_lcd_c::menu_pos_items(const menu_pos_st * menuPos)
{	// menu_pos_items
	uint8_t n=0;
...
void menu_lcd_c::doSettings(void)
{	// do Settings Menu
...
void menu_lcd_c::doCalibration(void)
{	// do Calibration Menu
	uint16_t lastUpdateDelta=menuTime-lastUpdate;
...
void menu_lcd_c::doRemote(void)
{ // doRemote
	uint16_t lastUpdateDelta=menuTime-lastUpdate;

	if ((lastUpdateDelta>UPDATE_CYCLE) || force_update)
	{	// update voltage and current readings
		// Update voltage & current settings and readings
		lastUpdate=menuTime;
		if (controller->is_constant_current_active())
...
void menu_lcd_c::setOptions(void)
{	// setOptions
	uint16_t Cal,Ref;
	uint16_t u;
	uint16_t i=5000;
